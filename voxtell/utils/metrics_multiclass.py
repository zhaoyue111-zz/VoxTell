...
def compute_metrics_from_label_map(pred, gt_label_map, eps=1e-7):
    """
    Compute per-class metrics without allocating a full one-hot ground truth.

    Args:
        pred: Binary predictions of shape (P, Z, X, Y).
        gt_label_map: Integer label map of shape (Z, X, Y), where class c uses
            label value c + 1.
        eps: Small epsilon value to avoid division by zero.
    """
    if pred.ndim != 4:
        raise ValueError(f"Expected 4D predictions (P,Z,X,Y), got {pred.ndim}")
    if pred.shape[1:] != gt_label_map.shape:
        raise ValueError(
            f"Shape mismatch: pred={pred.shape}, gt_label_map={gt_label_map.shape}."
        )
    ...
    for class_index in range(num_classes):
        ...
        gt_class = gt_label_map == (class_index + 1)
        ...
        dice[class_index] = (
            (2 * intersection + eps) / (pred_sum + gt_sum + eps)
        )
        iou[class_index] = (intersection + eps) / (union + eps)
