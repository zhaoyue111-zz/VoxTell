...
class VoxTellPredictor:
    ...
    def predict_single_image_with_alignment(
        ...
        # Foreground segmentation for alignment
        segmentation_mask = torch.sigmoid(predicted_logits.float()) > 0.5
        ...
        if output_type == "logits":
            output_tensor = predicted_logits.to("cpu").float()
        elif output_type == "probabilities":
            output_tensor = torch.sigmoid(predicted_logits.float()).to("cpu")
        else:
            output_tensor = segmentation_mask.to("cpu")

        output_np = output_tensor.numpy()
        output_dtype = np.uint8 if output_type == "binary" else np.float32
        segmentation_reverted_cropping = np.zeros(
            [output_np.shape[0], *orig_shape],
            dtype=output_dtype,
        )
        segmentation_reverted_cropping = insert_crop_into_image(
            segmentation_reverted_cropping, output_np, bbox
        )
